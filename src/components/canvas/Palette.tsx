...
type Props = { onAdd: (type: NodeType) => void; hasSelection: boolean; className?: string };

/** Rad med bokstyper. Klikk legger til en boks, koblet fra den valgte hvis noen er valgt. */
export function Palette({ onAdd, hasSelection, className }: Props) {
  return (
    <div
      role="group"
      aria-label={hasSelection ? "Legg til boks koblet fra den valgte" : "Legg til boks"}
      className={cn("flex max-w-full gap-1.5 overflow-x-auto rounded-md border border-border bg-card p-1.5 shadow-sm", className)}
    >
      {NODE_TYPES.map((t) => (
