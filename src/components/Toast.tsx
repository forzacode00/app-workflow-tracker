...
export function Toast({ toast, onDismiss }: Props) {
  return (
    <div
      role="status"
      aria-live="polite"
      className={`fixed bottom-4 left-1/2 z-20 flex w-max max-w-[calc(100vw-1.5rem)] -translate-x-1/2 items-center gap-3 rounded-md bg-foreground px-4 py-2 text-[13.5px] text-background shadow-lg transition-opacity ${toast ? "opacity-100" : "pointer-events-none opacity-0"}`}
    >
