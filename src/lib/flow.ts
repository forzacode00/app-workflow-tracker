...
export type NodeMeta = {
  ...
  briefTitle: string;
  /** Hvilke typer det er naturlig å legge til fra denne boksen. */
...
export const seedFlow = (): Flow => ({
  ...emptyFlow(),
  nodes: [{ id: "maal", type: "maal", tittel: "", notat: "", x: 0, y: 0 }],
});

export const isBlank = (f: Flow): boolean =>
  f.navn.trim() === "" && f.edges.length === 0 && f.nodes.every((n) => n.tittel.trim() === "" && n.notat.trim() === "");

/** Fjerner kanter som peker på bokser som ikke finnes, og duplikater. */
