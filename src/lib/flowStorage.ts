...
import { loadWorkflow as loadV1, type StorageLike } from "./storage";
...
export function parseFlow(json: string): ParseResult {
  ...
  if (obj.versjon !== 2 && Array.isArray(obj.steg)) {
    ...
    return v1.status === "ok" ? { ok: true, flow: migrateV1(v1.workflow) } : { ok: false, error: "Flyten fra det gamle skjemaet kunne ikke leses." };
  }
  const result = flowSchema.safeParse({ ...emptyFlow(), ...obj });
  if (!result.success) {
...
export function loadFlow(storage: StorageLike | null = getStorage()): LoadResult {
  ...
  if (v1.status === "ok") return { status: "ok", flow: migrateV1(v1.workflow) };
  return { status: "empty" };
}
...
export function saveFlow(flow: Flow, storage: StorageLike | null = getStorage()): boolean {
